...
Public Class frmrepairoba
    ...
    Public Sub InsertNG()
        ...
        cmd.CommandText = "INSERT INTO gi_repairobang(`pcbid`, `timestamp`, `judgement`, `defectname`, `remarks`, `operator`, `line`) VALUES ('" & txtScan.Text & "', NOW(), 'ng', '" & cmbdefectname.Text & "','" & txtremarks.Text & "', '" & lblname.Text & "', '" & lblline.Text & "')"
        cmd.ExecuteNonQuery()
    End Sub
    ...
    Public Sub gridout()
        ...
        If lblScan.Text = "SCAN PCB:" Then
            cmd.CommandText = "SELECT pcbid, obatimestamp, obaoperator, obastatus FROM gi_repairtrace WHERE  obatimestamp > (NOW() - INTERVAL 15 MINUTE)"

            myDA.Fill(myDT)
            dgpcb.DataSource = myDT
        Else
            cmd.CommandText = "SELECT pcbid, obatimestamp, obaoperator, obastatus FROM gi_repairtrace WHERE obatimestamp > (NOW() - INTERVAL 15 MINUTE) AND obatimestamp = 'ng'"
            myDA.Fill(myDT)
            dgpcb.DataSource = myDT
        End If
    End Sub
    ...
    Private Sub txtScan_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtScan.KeyPress
        ...
        If Asc(e.KeyChar) = 13 Then
            Try
                ...
                If lblScan.Text = "SCAN PCB:" Then
                    cmd.CommandText = "SELECT COUNT(pcbid) FROM gi_repairtrace WHERE pcbid = '" & txtScan.Text & "' AND processtoken = 'fvi' AND fvistatus = 'good'"
                    If cmd.ExecuteScalar = 1 Then

                        Select Case ExaminePCB(txtScan.Text)
                            Case "valid"
                                cmd.CommandText = "UPDATE gi_repairtrace SET `processtoken` = 'oba', obatimestamp = NOW(), obaoperator = '" & lblname.Text & "', obastatus = 'good' 
                        ...
                        End Select
                    Else
                        cmd.CommandText = "SELECT COUNT(pcbid) FROM gi_repairtrace WHERE pcbid = '" & txtScan.Text & "' AND (processtoken = 'fvi' OR processtoken = 'oba')"
                        If cmd.ExecuteScalar = 1 Then
                        Else
                ...
                Else
                    ...
                    Else
                        cmd.CommandText = "UPDATE gi_repairtrace SET `obastatus` = 'ng', obatimestamp = NOW(), `obaremarks` = '" & txtremarks.Text & "', `processtoken` = 'oba', `obaoperator` = '" & lblname.Text & "'
                                                WHERE `pcbid` = '" & txtScan.Text & "' AND (`processtoken` = 'fvi' OR `processtoken` = 'oba') "
                        cmd.ExecuteNonQuery()
    ...
    Private Sub frmrepairoba_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        dbConnect()
        txtScan.Focus()
        Timer1.Enabled = False
        tblPnlMain.BackColor = Color.White
